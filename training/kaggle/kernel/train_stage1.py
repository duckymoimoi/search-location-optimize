"""Train and evaluate multilingual E5 for Hanoi POI Stage 1.
...
def find_input_dir(explicit: str | None) -> Path:
    if explicit:
        candidate = Path(explicit)
        if all((candidate / name).exists() for name in REQUIRED_INPUTS):
            return candidate
    ...
    for root in roots:
        if not root.exists():
            continue
        for query_file in root.rglob("queries_10k.parquet"):
            candidate = query_file.parent
            if all((candidate / name).exists() for name in REQUIRED_INPUTS):
                return candidate
...
def load_data(input_dir: Path, config: dict[str, Any]) -> tuple[list[dict], list[dict]]:
    ...
    queries = pq.read_table(input_dir / "queries_10k.parquet", columns=query_columns).to_pylist()
    corpus = pq.read_table(
        input_dir / "corpus_search_view.parquet", columns=corpus_columns
    ).to_pylist()

    if len(queries) != 10_000:
        raise ValueError(f"Expected 10000 queries, got {len(queries)}")
    if len(corpus) != 46_792:
        raise ValueError(f"Expected 46792 corpus rows, got {len(corpus)}")
    if any(row["dataset_version"] != config["dataset_version"] for row in queries):
        raise ValueError("Dataset version mismatch in query rows")
    ...
    if len(destination_corpus) != 45_693:
        raise ValueError(
            f"Expected 45693 searchable destinations, got {len(destination_corpus)}"
        )
...
def run_training(
    ...
    if selected["model_path"]:
        ...
        base_model = AutoModel.from_pretrained(
            ...
            revision=selected_model_config["model_revision"],
        )
