...
Đọc SYSTEM_DESIGN.md để hiểu kiến trúc; file này chốt hành vi code. `contracts/openapi.json` là contract HTTP; `configs/search.json` là giá trị mặc định; `contracts/model_bundle.schema.json` là contract bàn giao model. Config có thể đổi sau benchmark nhưng mỗi lần đổi phải tạo policy/release version.
...
| `vectors.npy` và `vector_ids.parquet` | matrix float32 [M,384], contiguous row_index → poi_id duy nhất |
...
N=50 là budget nội bộ mặc định; public top_k=1..5. Eval harness có final K=5/20/50 và access internal snapshots; không mở candidate depth/model choice cho public client.

## 4. Context và feature schema

`contracts/features.json` là thứ tự input cho tabular models. Feature factory dùng cùng code cho offline replay và serving. Missing numeric biểu diễn value=0 + missing flag riêng; không diễn giải khoảng cách thiếu là 0 m. Raw BM25 được giữ debug, model dùng reciprocal rank/bounded score và mask; không coi BM25 từ các scopes là cùng scale.
...
Heuristic v4 của demo là relevance-gated geo. Query rewrite, category intent, evidence depths và trọng số nằm trong `apps/poi-search/api/search_policy.json`, không hard-code trong Python. Candidate phải có lexical rank trong ngưỡng hoặc dense rank trong ngưỡng; khi query có entity intent rõ, category phải tương thích và các token phân biệt còn lại phải có evidence trong name/alias. Sau gate, relevance kết hợp normalized retrieval evidence và name-token coverage; khoảng cách chỉ là một thành phần blend, không phải khóa sort tuyệt đối. Exact name/alias được bảo vệ và kết quả dedup theo entity group. Không cố lấp đủ K bằng candidate yếu. Đây là baseline có chủ ý đơn giản, chưa phải learned personalization.

Profile `dense_only` là đối chứng chạy thật: không gọi lexical branch, query rewrite, token-overlap hoặc category-intent filter; chỉ dùng dense ANN rank và geo blend. Profile này phải có release/profile ID riêng và không được dùng metric hybrid để mô tả chất lượng. Live diagnostic hiện cho thấy dense-only nhanh hơn nhưng nhạy với viết tắt và semantic false positives; giữ ở port demo riêng cho đến khi đánh giá trên cùng benchmark được chốt.
...
| POST /v1/suggest | Query-only endpoint, không nhận origin/user/time |
...
UI khởi tạo bằng POST /v1/sessions, server tạo opaque session ID/cookie và namespace demo. Cookie HttpOnly, SameSite=Lax, Secure khi HTTPS; mọi session_id trong body phải khớp cookie. GET /v1/demo/users trả registry ID/label, không trả history. Khi triển khai ngoài demo phải tích hợp auth của ứng dụng. Authenticated subject lấy từ server session/token. `demo_user_id` và `context_time` chỉ được dùng khi server bật demo, chọn trong registry fixtures; production có các fields này khác null →403. Không để public client giả user để đọc history. Query-only benchmark không nhận các fields context nhờ additionalProperties=false.
...
MVP chỉ implement straight_line. Input origin/destination POI ID cùng corpus; origin null →status missing_origin. Geometry GeoJSON LineString giữa ranking points; geodesic_distance_m có giá trị, route_distance_m/route_duration_s=null. Chế độ road hoặc auto chỉ có road kết quả khi routing points verified và configured adapter/graph/profile hỗ trợ; hiện corpus chưa thỏa. Road strict không fallback →unavailable. Auto/allow_fallback →straight_line với lý do. Không gọi route cho mỗi candidate/keystroke; deadline riêng 1.000 ms. Road adapter phải giữ input/snapped points và graph version; car profile không tự được coi là xe máy.
